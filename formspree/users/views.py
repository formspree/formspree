...
def register():
    ...
    try:
        ...
        DB.session.add(user)
        DB.session.commit()
        g.log.info('User account created.', ip=request.remote_addr)
    except ValueError:
        DB.session.rollback()
        flash(u"{} is not a valid email address.".format(
            request.form['email']), "error")
        g.log.info('Account creation failed. Invalid address.', ip=request.remote_addr)
        return render_template('users/register.html')
    except IntegrityError:
        DB.session.rollback()
        flash(u"An account with this email already exists.", "error")
        g.log.info('Account creation failed. Address is already registered.', ip=request.remote_addr)
        return render_template('users/register.html')
...
def login():
    ...
    login_user(user, remember=True)
    g.log.info('Logged user in', user=user.email, ip=request.remote_addr)
